...
const ProfileModal = ({modal, setModal, instructor }) => {
  ...
  const handleProfileUpdate = (e) => {
    ...
    axios.patch('http://13.212.56.111:3001/v1/api/instructor/profile/', data)
      .then((response) => {
    ...
    if (imageFile) {
      ...
      axios.patch('http://13.212.56.111:3001/v1/api/instructor/profile/photo', formData, {
        headers: {
    ...
    const fetchPic = async () => {
      try {
        ...
        const picResponse = await axios.get(
          `http://13.212.56.111:3001/v1/api/s3/instructor/single/${instructorId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
