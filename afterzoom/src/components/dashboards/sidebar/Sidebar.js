...
const ProfileModal = ({ modal, setModal, student }) => {
  ...
  const handleProfileUpdate = (e) => {
    ...
    axios.patch('http://47.128.71.161:3001/v1/api/student/profile/', data, {
      headers: {
    ...
    if (imageFile) {
      ...
      axios.patch('http://47.128.71.161:3001/v1/api/student/profile/photo', formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    const fetchPic = async () => {
      try {
        ...
        const picResponse = await axios.get(
          `http://47.128.71.161:3001/v1/api/s3/student/single/${studentId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
