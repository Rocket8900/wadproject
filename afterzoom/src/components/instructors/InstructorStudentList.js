...
function InstructorLessonList() {
  ...
  useEffect(() => {
    const getInstructorsData = async () => {
      try {
        ...
        const response = await axios.get(`http://13.212.56.111:3001/v1/api/instructor/profile/${id}`, {
          headers: {
    ...
    const getPendingStudents = async () => {
      try {
        ...
        const response = await axios.get(`http://13.212.56.111:3001/v1/api/booking/instructor`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  useEffect(() => {
    const fetchStudentProfiles = async () => {
      ...
      const profilePromises = pendingBookings.map(async (booking) => {
        try {
          const response = await axios.get(`http://13.212.56.111:3001/v1/api/student/profile/${booking.studentId}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
  ...
  const acceptBooking = async (bookingId) => {
    try {
      const token = Cookie.get('access_token');
      await axios.patch(`http://13.212.56.111:3001/v1/api/booking/${bookingId}`, {
        status: "ACCEPTED",
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
