...
const LoginModal = ({ isOpen, setIsOpen }) => {
  ...
  const handleStudentLogin = async () => {
    try {
      const response = await axios.post('http://47.128.71.161:3001/v1/api/student/login', {
        email,
        password,
  ...
  const handleInstructorLogin = async () => {
    try {
      const response = await axios.post('http://47.128.71.161:3001/v1/api/instructor/login', {
        email,
        password,
      }, {withCredentials: true})
