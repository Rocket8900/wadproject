...
function InstructorChatPage({instructor}) {
    ...
    const fetchStudents = async () => {
      try {
        const response = await axios.get(
          "http://13.212.56.111:3001/v1/api/chat/instructor/list",
          {
            headers: {
        ...
        for (let i = 0; i < allChat.length ; i ++) {
          ...
          const profile = await axios.get(`http://13.212.56.111:3001/v1/api/student/profile/${allChat[i]}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
          ...
          if (profileDetails.data.selfie == null) {
            ...
            profileDetails.data.selfie = (await axios.get(`http://13.212.56.111:3001/v1/api/s3/student/single/${profileDetails.data.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                  }
            })).data.data
  ...
  const handleStudentChange = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://13.212.56.111:3001/v1/api/chat/${e}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
