...
function StudentChatPage({student}) {
    ...
    const fetchInstructors = async () => {
      try {
        const response = await axios.get(
          "http://47.128.71.161:3001/v1/api/chat/student/list",  // Assuming the endpoint
          {
            headers: {
        ...
        for (let i = 0; i < allChat.length; i++) {
          const profile = await axios.get(`http://47.128.71.161:3001/v1/api/instructor/profile/${allChat[i]}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
          ...
          if (profileDetails.data.dp == null) {
            ...
            profileDetails.data.dp = (await axios.get(`http://47.128.71.161:3001/v1/api/s3/instructor/single/${profileDetails.data.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                  }
            })).data.data
  ...
  const handleInstructorChange = async (e) => {
    ...
    try {
      const response = await axios.get(
        `http://47.128.71.161:3001/v1/api/chat/${e}`,  // Assuming this endpoint gets the chat history
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
